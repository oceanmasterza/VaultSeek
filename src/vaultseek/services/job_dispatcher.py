...
class JobDispatcher:
    def __init__(
        ...
        *,
        ...
        self._scan_pool = ThreadPoolExecutor(
            max_workers=self._scan_max, thread_name_prefix="vaultseek-scan"
        )
        # Shared CPU pool for hash_file and fingerprint_file.
        # Frozen Windows builds use threads — ProcessPoolExecutor relaunches
        # VaultSeek.exe per worker and flashes console windows; threads keep
        ...
        self._metadata_pool = ThreadPoolExecutor(
            max_workers=self._meta_max, thread_name_prefix="vaultseek-meta"
        )
        ...
        if self._cpu_pool is None:
            ...
            if frozen and sys.platform == "win32":
                ...
                self._cpu_pool = ThreadPoolExecutor(
                    max_workers=workers, thread_name_prefix="vaultseek-cpu"
                )
            else:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._poll_loop, name="vaultseek-dispatcher", daemon=True
        )
        self._thread.start()
