"""SQLite engine factory with VaultSeek's required PRAGMA configuration.
...
def _configure_sqlite_connection(dbapi_connection: sqlite3.Connection, _record: Any) -> None:
    """Apply VaultSeek's required PRAGMAs to a newly opened connection.
...
def create_sqlite_engine(db_path: Path, *, echo: bool = False) -> Engine:
    """Create a SQLAlchemy engine for the VaultSeek database at ``db_path``.

    Every connection the returned engine opens has WAL mode, foreign key
    enforcement, and the rest of VaultSeek's PRAGMA configuration
