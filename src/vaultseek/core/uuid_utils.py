...
_stdlib_uuid7 = getattr(_uuid_module, "uuid7", None)
...
def generate_uuid7() -> UUID:
    ...
    if _stdlib_uuid7 is not None:
        return cast(UUID, _stdlib_uuid7())
    return _uuid7_polyfill()


# Public alias so call sites / tests can ``from vaultseek.core.uuid_utils import uuid7``.
