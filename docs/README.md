# VaultSeek documentation

Start here when onboarding or before architectural work.

**AI assistants:** read [../AGENTS.md](../AGENTS.md) first.

## Essential reading (order)

1. [../AGENTS.md](../AGENTS.md) — third-party AI onboarding (repo map, waterfall, Settings vs Plugins, GitLab `main`)
2. [../README.md](../README.md) — project landing page
3. [USER_GUIDE.md](USER_GUIDE.md) — first run, plugins, troubleshooting
4. [PROWLARR.md](PROWLARR.md) — Prowlarr tiers + qBittorrent + SABnzbd
5. [NICOTINE_PLUS.md](NICOTINE_PLUS.md) — Nicotine+ provider setup (HTTP vs socket)
6. [ARCHITECTURAL_UPDATE_001.md](ARCHITECTURAL_UPDATE_001.md) — Acquisition Engine model
7. [ARCHITECTURE.md](ARCHITECTURE.md) — layers and pipelines
8. [PROJECT_PLAN.md](PROJECT_PLAN.md) — product vision (historical)
9. [DECISIONS.md](DECISIONS.md) — ADRs
10. [ROADMAP.md](ROADMAP.md) — public roadmap
11. [DEVELOPMENT_ROADMAP.md](DEVELOPMENT_ROADMAP.md) — internal engineering notebook
12. [AI_RULES.md](AI_RULES.md) — coding and documentation rules
13. [SOURCES.md](SOURCES.md) — acquisition sources and future-provider ideas
14. [TECH_STACK.md](TECH_STACK.md) — languages, libraries, tooling

## Terminology (use consistently)

| Use | Avoid as architecture name |
|-----|----------------------------|
| Acquisition Engine | Search Engine |
| AcquisitionJob | — |
| Provider | Soulseek app / downloader |
| Verification Pipeline | — |
| Import Pipeline | — |

## Detailed architecture (MusicVault heritage)

The `architecture/` folder contains deeper design docs from the MusicVault fork. They are being aligned with the Acquisition Engine model; when in doubt, prefer **ARCHITECTURAL_UPDATE_001.md** and **DECISIONS.md**.
